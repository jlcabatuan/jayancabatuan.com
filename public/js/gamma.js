...
var Gamma = (function() {
	...
	var $window = $( window ),
		...
		_createSingleViewNavigation = function() {
			...
			//
			// }
			console.log('_createSingleViewNavigation')
		...
		_layout = function( $items ) {
			...
			$items.each( function() {
				...
				var $item = $( this ),
					...
					caption = $picEl.data( 'caption' ),
					exif = $picEl.data( 'exif' );
				...
				// console.log(description, caption, exif);
				// $( '<div/>' ).addClass( 'gamma-description' ).html( description ).insertAfter( $picEl );
				var $foo = $( '<div/>' ).addClass( 'gamma-description' ).html( description ).insertAfter( $picEl );
				var $bar = $( '<div/>' ).addClass( 'gamma-caption' ).html( caption ).insertAfter( $foo );
				$( '<div/>' ).addClass( 'gamma-exif' ).html( exif ).insertAfter( $bar );
		...
		_singleview = function() {
			...
			// var id = $( this ).index();
			// _saveState( id );
			console.log('_singleview')
		},
		// shows the item
		_singleviewitem = function( $item, anim ) {
			console.log('_singleviewitem')
			// Gamma.isSV = true;
			//
			// var id = $item.index(),
			// 	data = $item.data(),
		...
		_loadSVItemFromGrid = function( data, position, src ) {
			console.log('_loadSVItemFromGrid')
			// show single view
		...
		_getFinalSizePosition = function( imageSize, wrapperSize ) {
			console.log('_getFinalSizePosition')
			// image size
